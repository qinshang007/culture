...
public class FpTree {
    ...
    private static int support = 3;		//置信度
    
    private static int size = 2;		//频繁项级大小
     
    public static List<String> getFrequnceItems(List<List<String>> items) throws IOException{
        //从文件中读取事物数据集
        List<List<String>> transactions = items;
        //初始一个频繁模式集
        List<String> frequences = new LinkedList<String>();
        List<String> results = new LinkedList<String>();
        //开始递归
        digTree(transactions,frequences,results);
        return results;
    }
     
    public static void digTree(List<List<String>> transactions,
            List<String> frequences,List<String> results){
        //扫描事物数据集，排序
        ...
        for(int i=headTable.size()-1;i>=0;i--){
            ...
            if(nextFrequences.size() > 1){
            	String result = StringUtils.join(nextFrequences,",")+"\t"+sortedMap.get(subject);
                System.out.println(result);
                results.add(result);
            }
             
            digTree(frequentModeBases,nextFrequences,results);
