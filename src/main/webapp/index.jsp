...
<body class="page-header-fixed">
						...
						<div class="portlet-body form">
							...
							<form id="dtForm" action="/culture/doctor/save.do" class="form-horizontal" method="post" enctype="multipart/form-data" target="hidden_frame">
								...
								<div class="control-group">
									...
									<div class="controls">
										<input id="hpname" type="text" class="span6 m-wrap" name="dtname" />
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" class="span6 m-wrap" name="level" />
										<span class="help-inline">必填</span>
									</div>
								</div>
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" class="span6 m-wrap" name="sort" />
										<span class="help-inline">一个0~10数字，代表排序优先级</span>
									</div>
								</div>
								<div class="control-group">
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										<select id="hpid" class="span6 chosen" data-placeholder="选择一个医院" tabindex="1" name="hpid">
											<option value="<c:if test="${not empty hpid}">${hpid}</c:if>"><c:if test="${not empty hpname}">${hpname}</c:if></option>
											<c:forEach  items="${hplist}"  var="item"  varStatus="status">
												<option value="${item.hpid}">${item.hpname}</option>
											</c:forEach>
										</select>
										<span class="help-inline">必填</span>
									</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input id="address" type="text" class="span6 m-wrap" name="skill" />
										<span class="help-inline">必填</span>
									</div>
								</div>
								<div class="control-group">
									...
									<div class="controls">
										<textarea class="span6 m-wrap" rows="3" name="about"></textarea>
									</div>
								</div>
	...
	<script>
		jQuery(document).ready(function() {       
		   ...
		   App.init();
		   var rule={dtname:{required:true},hpid:{required:true},level:{required:true},sort:{number:true}};
		   FormValidation.init($('#dtForm'),rule,creatSubmitForm('dtForm'),failedForm);
		});
		...
		function creatSubmitForm(formId){
			return function submitForm(){
			    ...
			    success: function(transport)
			    {
			    	 ...
			    	 location.href="/culture/doctor/dtAdd.do?hpid="+$("#hpid").val()+"&hpname="+$("#hpid option:selected").text();
			    },
